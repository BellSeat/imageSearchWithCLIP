...
ffmpegLogger = setup_local_logger('logs/ffmpeg.log') # Corrected typo: ffempg.log to ffmpeg.log
...
def check_ffmpeg():
    try:
        ...
        logger.error("ffmpeg is not installed. Please install ffmpeg to use video processing features.")
        return False
...
def get_keyframe_timestamps(video_path: str)->list: # Renamed for clarity
    ...
    try:
        result = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=True) # Added check=True
        if result.returncode != 0:
            raise RuntimeError(f"ffprobe error: {result.stderr}")
        timestamps = []
        for line in result.stdout.splitlines():
            parts = line.split(',')
            if len(parts) >= 3 and parts[0] == 'frame' and parts[2] == 'I': # Only 'I' frames for keyframes
                try:
...
def extract_keyframes(video_path, output_dir): # Renamed to extract_keyframes, returns paths
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    if not check_ffmpeg():
        raise RuntimeError("FFmpeg is not installed, cannot extract keyframes.")
    ...
    for i, ts in enumerate(timestamps, start=1):
        ...
        try:
            ...
            ffmpegLogger.info(f"FFmpeg output for {output_file}: {results.stdout.strip()}")
            if results.stderr: # Log stderr for warnings/non-fatal errors
                 ffmpegLogger.warning(f"FFmpeg stderr for {output_file}: {results.stderr.strip()}")
            extracted_frame_paths.append(output_file)
        except subprocess.CalledProcessError as e:
            logger.error(f"Error extracting keyframe {output_file}: {e.stderr}", exc_info=True)
        except Exception as e:
            logger.error(f"Unexpected error during keyframe extraction for {output_file}: {e}", exc_info=True)
    logger.info(f"Finished extracting {len(extracted_frame_paths)} keyframes to {output_dir}")
...
def build_output_dir(video_path):
    ...
    total_frames = get_video_info(video_path) # total_frames could be None if video fails to open
...
def clip_video_segment(original_video_path: str, timestamp_s: float, output_path: str, duration_s: int = 4):
    ...
    """
    if not check_ffmpeg():
        raise RuntimeError("FFmpeg is not installed, cannot clip video segments.")
    ...
    try:
        ...
        logger.error(f"Could not get video duration for {original_video_path}: {e}", exc_info=True)
    ...
    logger.info(f"Clipping video {original_video_path} from {start_clip:.2f}s for {duration_s:.2f}s to {output_path}")
    try:
        results = subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        ...
        logger.error(f"Unexpected error during video clipping: {e}", exc_info=True)
