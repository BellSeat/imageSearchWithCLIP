...
def main():
    ...
    DEFUALT_IMAGE_PATH = os.path.join(CURRENT_DIR, "..", "database", "raw", "video",
                                      "frames", "sample_2025-05-16_01-06-10_frames_1200", "frame_0004_0-00-04-504500.jpg")
    parser = argparse.ArgumentParser(
        description="Process images/video and interact with vector database.")
    parser.add_argument(
        "--image", type=str, help="Path to a single image file to embed and add to DB.")
    parser.add_argument("--image_folder", type=str,
                        help="Path to a folder containing images to embed and add to DB.", default=DEFUALT_IMAGE_PATH)
    parser.add_argument(
        "--text", type=str, help="Text description (optional) for single image.", default="")
    parser.add_argument("--search_text", type=str,
                        help="Text query to search for in the database.", default="")
    parser.add_argument("--search_image", type=str,
                        help="Path to an image file to use as a query for searching the database.", default=DEFUALT_IMAGE_PATH)
    parser.add_argument("--config", type=str,
                        help="Path to config.json", default=CONFIG_PATH)
    ...
    try:
        ...
        print(
            f"[CRITICAL ERROR] Failed to initialize Embedding or VectorDatabase: {e}")
    ...
    if args.image_folder:
        if os.path.exists(args.image_folder):
            ...
            for filename in os.listdir(args.image_folder):
                ...
                if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')):
                    ...
                    if image_embedding is not None:
                        ...
                        image_metadata = {
                            "path": image_path, "text": f"Image from {args.image_folder}"}
            ...
            if batch_embeddings:
                print(
                    f"[INFO] Adding {len(batch_embeddings)} embeddings to database in batch...")
                ...
                print(
                    f"[✅] Successfully processed {processed_count} images from folder and added to database.")
            else:
                print(
                    f"[ℹ️] No images were successfully embedded from folder: {args.image_folder}")

            if failed_count > 0:
                print(
                    f"[⚠️] Failed to process {failed_count} images from folder.")
    ...
    if args.text and not args.image and not args.image_folder and not args.search_text and not args.search_image:
        print(
            f"[INFO] Generating embedding for standalone text: \"{args.text}\"")
    ...
    if args.search_text:
        print(
            f"[INFO] Performing search for text query: \"{args.search_text}\"")
        ...
        if query_embedding is not None:
            ...
            if search_results:
                ...
                print(
                    f"[ℹ️] No results found for \"{args.search_text}\". Please ensure the database contains relevant embeddings.")
        else:
            print(
                "[❌] Failed to create embedding for search text query. Is the text valid?")

    # Perform search if search_image is provided
    if args.search_image:
        if os.path.exists(args.search_image):
            print(
                f"[INFO] Performing search for image query: \"{args.search_image}\"")
            ...
            if query_embedding is not None:
                ...
                if search_results:
                    print(
                        f"[✅] Search results for image \"{args.search_image}\":")
                    for path, distance in search_results:
                        print(f"  - Path: {path}, Distance: {distance:.4f}")
                else:
                    print(
                        f"[ℹ️] No results found for image \"{args.search_image}\". Please ensure the database contains relevant embeddings.")
            else:
                print(
                    "[❌] Failed to create embedding for search image query. Is the image path valid?")
        else:
